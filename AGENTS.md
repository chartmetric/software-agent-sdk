...
## Repository Memory
- Chat completion responses that finish with `stop` but contain no answer or tool call are retried inside `_validate_chat_response`, after usage is recorded. LiteLLM can normalize a provider's `finish_reason=error` to `stop`; accepting its partial reasoning polluted Pilot's conversation 8e1fc0a9 with corrective feedback instead of retrying the unchanged request. Keep length-limit and content-filter responses out of this retry rule.
